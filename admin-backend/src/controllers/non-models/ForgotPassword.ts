...
import { TControllerReturnType, UserType } from "../../types";
...
export class ForgotPasswordClass<T extends { _id: string; emailVerified: boolean; email: string; firstName: string }> {
    ...
    public static reset<Y extends UserType>({ model, link }: { model: BaseModel<Y>; link: string }): Promise<TControllerReturnType> {
